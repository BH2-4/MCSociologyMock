...
### 7.2 P1：【绝区零】42 天角色全球发行实验

P1 不扩张为通用游戏社会，而是复用 P0 的预注册、配对分支、关系传播、事件账本和多 Provider Adapter，针对米哈游赛道的一个真实约束做下一轮验证：**在 42 天版本周期内，使用届时已经公开的《绝区零》下一版本资料，对一个目标发行角色的全球发行定位做可复现的区域化预演。**

首个实验问题固定为：

> 在角色事实、官方素材、卡池规则、发布时间、渠道、首轮受众和本地化质量均保持一致时，首屏信息采用“战斗价值优先”还是“角色/叙事吸引力优先”，是否会在不同语言/区域的合成玩家网络中改变信息传播、抽取计划和模拟付费货币支出？

唯一处理变量为：

`message_positioning = COMBAT_VALUE_FIRST | CHARACTER_AFFINITY_FIRST`

两个分支只能调整相同事实的排序与表达重心，不得改变技能数值、角色设定、卡池时间、价格、赠送资源、素材质量或渠道曝光。若公开资料不足以生成两个事实等价的版本，则该实验不得启动，必须改用另一项可由公开资料支持的单变量干预。

| 赛题约束 | P1 要替代的一周目方式 | AI 二周目方式 |
|---|---|---|
| 42 天版本周期 | 依赖人工汇总、经验讨论和发布后的复盘，难在决策窗口内重复多个受控情景 | 从公开信息生成冻结快照，在 24 小时内完成多语言配对模拟、机制追踪和失败报告 |
| 多语种同步 | 各语言素材分别处理，难确认差异来自发行策略还是翻译偏移 | 先建立语言无关的事实槽与处理变量，再生成本地化版本并通过语义等价门禁 |
| 区域玩家差异 | 用全球汇总互动量掩盖区域方向相反或阻力位置不同 | 在同一协议下预注册语言/区域子群，分别报告曝光、传播、抽取计划和模拟支出差值 |
| 只使用公开数据 | Wiki、公告、视频和社区信号分散，容易混入发布时间之后的信息 | 建立带 URL、语言、发布时间、采集时间、内容哈希和许可状态的 Public Source Bundle |

P1 数据边界：

- `T0` 为下一版本首次正式公开的时间；只摄取 `data_cutoff_at` 之前可公开访问的官方公告、特别节目/预告、角色演示、游戏官网与 HoYoLAB 页面、应用商店页面、公开 Wiki，以及遵守平台规则采集的公开社区聚合信号；
- 禁止使用泄露、拆包未公开内容、私密群聊、真实玩家账号明细、内部投放数据或内部流水；每个事实必须引用 `source_id`，无法溯源的角色信息不得进入 Agent observation；
- 至少选择 3 个公开数据覆盖充分的语言/区域群组。区域标签只表示本次合成人群的分析分层，不声称代表该地区真实玩家；
- 角色状态至少包含 `roster_need`、`combat_preference`、`character_affinity`、`pull_budget`、`guarantee_state`、`spend_propensity`、`source_trust` 和已见发行信息；参数来源、假设分布与版本必须可导出；
- 本地化版本必须共享同一事实槽、信息量、素材清单和发布时序；专有名词、数值、否定关系或承诺发生偏移时，Localization Gate 拒绝运行；
- 不建立或操作真实游戏账号，不执行真实抽卡或充值。默认使用 Synthetic Spend Ledger；可选 x402 测试网模式只用于审计模拟付款事件，不得称为游戏内购买或真实流水。

| ID | P1 功能 | 需求与验收标准 |
|---|---|---|
| P1-ZZ-01 | Version Snapshot | 在 `T0 + 6h` 内冻结 `version_id`、目标 `character_id`、公开事实、素材、卡池规则、数据截止时间和 `public_source_bundle_hash` |
| P1-ZZ-02 | 发行实验预注册 | 锁定唯一处理变量、语言/区域分组、主要指标、数据截止、替代解释和失败判据；禁止看到结果后改写角色卖点 |
| P1-ZZ-03 | 合成玩家网络 | 生成语言/区域、现役/回流/潜在玩家和玩法偏好的异质 Agent；区域间桥接关系与信息渠道可追溯 |
| P1-ZZ-04 | 多语言等价门禁 | 对两个策略版本逐事实槽比对；除信息排序和预注册表达重心外，事实、强度、素材与曝光必须一致 |
| P1-ZZ-05 | 角色发行行动 | Agent 可查看官方信息、询问、讨论、转发、`PLAN_PULL`、`SAVE`、`SKIP` 或 `SIMULATED_TOP_UP`；所有行动引用实际曝光事件 |
| P1-ZZ-06 | 流水代理指标 | 主要结果为非种子合成玩家的 `simulated_character_spend` 平均配对差；同时报告计划抽取率、模拟付费转化、传播深度、决策延迟和区域异质性 |
| P1-ZZ-07 | 结果边界 | 只输出模型内相对效应、方向稳定性和机制漏斗；禁止输出“预计真实流水 X 元”或将模拟支付等同于《绝区零》真实收入 |
| P1-ZZ-08 | 42 天交付节奏 | `T0 + 24h` 内产出首轮配对结果；公开资料变更时创建新 Snapshot，不覆盖旧协议，保证同一版本内可回放、跨版本可比较 |

`simulated_character_spend` 只计算固定决策窗口内由目标角色触发的合成充值单位；免费赠送资源、Agent 已持有资源和未执行的抽取计划分别记录，不得并入流水代理。该指标没有法币含义，也不能由测试网 USDC 面额换算成真实收入。

这里的“影响角色流水”严格指对**模拟角色付费指数及其前置行为路径**做实验。由于公开数据通常不能识别单角色真实流水，应用价值是提前比较发行策略、定位区域性反转与生成待真人验证的假设，而不是预测财务结果。[R3][R4][R5][R7][R8]
...
官方 middleware 接受静态 `RouteMap`。P0 在 Run 启动前固定 `offer_eco_cup` 的路由、价格、资产与 `payTo`；运行中不得修改付款要求。动态报价与运行时改价留到 P1。
...
6. **外部验证**：P1 使用真人问卷、小样本行为实验或真实交易数据比较条件效应。
...
社会科学联网调查先使用博查 Web Search API 做候选发现，再以 DOI、出版社、正式会议页面和开放全文核验；完整查询、筛选与版本替换记录见 R8-R10。Injective 技术事实只采用官方博客、官方开发文档和 InjectiveLabs 发布的 npm 包。x402 文档页面未单独标注发布日期；官方 sitemap 显示该页最近更新于 2026-06-07。网页访问日期均为 2026-07-24。
