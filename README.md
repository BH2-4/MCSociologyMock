...
A local, ignored 300-wallet Injective testnet bundle was structurally and cryptographically verified. Chain audit on `eip155:1439` found all 300 wallets funded with 0.002 testnet INJ and five wallets funded with 1 testnet USDC. Four funded wallets are configured as branch-isolated seeds, a fifth as the facilitator, and a distinct sixth address as merchant; no secret is committed. All four seed payments are confirmed in `fixtures/testnet-seed-receipts.json`: every seed now holds 0.70 testnet USDC, the merchant received 1.20 testnet USDC, and all four bounded Evidence records validate. A testnet-backed paired run restored Evidence-blind `0` and Fixed-threshold `+4/22`. PostgreSQL migration/save/read/Replay has also been exercised against the Compose database. The Token Plan key is configured only in the ignored local environment. Two full online `MiniMax-M2.7` paired attempts were made with client concurrency fixed at one and Control/Treatment executed serially. The first failed directly on provider HTTP `529`; after a minimal health probe returned `200`, the second ended on `fetch failed`. Neither attempt persisted a partial pair, switched models, retried the whole Run automatically, signed, or paid. A supplemental one-Agent paired validation then passed after adopting MiniMax's official tool-call structure and a sufficient completion budget; its redacted report is in `fixtures/online-mini-validation.json`. It does not replace the still-required successful 24-Agent online Run.
