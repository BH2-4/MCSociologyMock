...
2. The MiniMax Token Plan Base URL, `MiniMax-M2.7` model and a rotated subscription key are configured only in the ignored local environment. Client concurrency remained one and the Runner completed Control before starting Treatment. A full testnet-backed attempt with Seed `minimax-online-20260725-01` ran for 10 minutes 47 seconds before provider HTTP `529`. A minimal availability probe then returned `200`, so the same Seed was retried once; that full attempt ended after about eight minutes on `TypeError: fetch failed`. Both attempts failed directly as required and persisted no partial pair. A subsequent one-Agent paired diagnostic exposed repeated 500-token truncation. The Adapter was aligned with MiniMax's documented tool-call response and `max_completion_tokens`; the final diagnostic then completed both branches on their first attempt with frozen-observation parity, zero Control leakage, one valid Treatment Evidence citation, zero invalid references/actions and zero side effects. Its redacted report is `fixtures/online-mini-validation.json` (`1431` prompt and `1324` completion tokens). This diagnostic does not substitute for a successful 24-Agent online Run, which remains unverified.
